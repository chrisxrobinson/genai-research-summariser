...
class Settings(BaseSettings):
    ...
    dynamodb_endpoint: str = os.environ.get(
        "DYNAMODB_ENDPOINT", None
    )  # For local development
    # S3 Configuration
    s3_bucket_name: str = os.environ.get(
    ...
    vector_db: Literal["pinecone", "qdrant"] = os.environ.get(
        "VECTOR_DB", "pinecone"
    )
    # Pinecone Configuration (for production)
    pinecone_api_key: str = os.environ.get("PINECONE_API_KEY", "")
    pinecone_environment: str = os.environ.get(
        "PINECONE_ENVIRONMENT", "us-west1-gcp"
    )
    pinecone_index_name: str = os.environ.get(
        "PINECONE_INDEX_NAME", "research-embeddings"
    )
    # Qdrant Configuration (for local development)
    qdrant_url: str = os.environ.get("QDRANT_URL", "http://localhost:6333")
    qdrant_collection_name: str = os.environ.get(
        "QDRANT_COLLECTION", "research-embeddings"
    )
    # Embedding Configuration
    embedding_dimension: int = int(
        os.environ.get("EMBEDDING_DIMENSION", "1536")
    )
    # Mistral API
